import apolloimg from '../images/apolloimg.png';
import css from '../images/css.webp';
import github from '../images/githhub.webp';
import html5s from '../images/html5.webp';
import jsImg from '../images/js.webp';
import reactimg from '../images/reactimg.webp';
import resticon from '../images/resticon.webp';
import nodeIcon from '../images/node-icon.webp';
import graphql from '../images/graphql.webp';
import sql3 from '../images/sql3.webp';
import heroku from '../images/heroku.webp';
import python from '../images/python.webp';
import mongo from '../images/mongo.webp';
import sass from '../images/sass.webp';
import aws from '../images/aws.webp';

const skillCards = [
	{
		title: 'Html5',
		img: html5s,
		description:
			'Experince using html to seperate web elements into appropriate html tags and display data',
	},
	{
		title: 'CSS',
		img: css,
		description:
			'Experience in using css to make UI with and without a UX designer',
	},
	{
		title: 'Sass',
		img: sass,
		description:
			'Experience using the sass css framework to handle scalable styling for big apps',
	},
	{
		title: 'Javascript',
		img: jsImg,
		description:
			'Experince writing javascript es6, DOM manipulation, writing functions, bulit in methods,',
	},
	{
		title: 'Python',
		img: python,
		description:
			'Learned computer science fundamentals with hands on projects using python, as well as experience in Django and Flask',
	},
	{
		title: 'React',
		img: reactimg,
		description:
			'The bulk of my experience on building front end is in the React framework utilzing popular libraies',
	},
	{
		title: 'Node',
		img: nodeIcon,
		description: 'Experience building node servers withe express framwork',
	},
	{
		title: 'AWS',
		img: aws,
		description:
			'Hands on experience with codepipeline, cloud formation, s3, aws neptune',
	},
	{
		title: 'REStful apis',
		img: resticon,
		description:
			'Experince in working with and building restful api endpoints to build full crud functionality',
	},
	{
		title: 'Apollo',
		img: apolloimg,
		description:
			'Experience with Apollo servers using apollo gateway/federation on back end and apollo client store on the front end',
	},
	{
		title: 'Github',
		img: github,
		description:
			'Experience working with online cloud version control utilizing seperate production, staging, development, feature branches',
	},
	{
		title: 'Graphql',
		img: graphql,
		description:
			'Experience building graphql apis writing resovers, queries, mutations',
	},
	{
		title: 'Sql',
		img: sql3,
		description:
			'Experience using postgres databases tables, queries, joins',
	},
	{
		title: 'MongoDB',
		img: mongo,
		description:
			'Experience using mongo to create a noSql database and rest api using mongoose',
	},
	{
		title: 'Heroku',
		img: heroku,
		description:
			'Experince with live deployment/configuration using heroku linked with a github repository master branch',
	},
];

export default skillCards;
